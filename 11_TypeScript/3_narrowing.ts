...
// 아직 이파라미터의 타입이 확실하지 않으니까 자료 조작을 막음
...
// x라는 파라미터는 옵션이고 옵션인 파라미터 number | undefined 식으로 타입 정의가됨
// 그래서 아직 x라는 파라미터가 뭔지 확실하지 않기 떄문에 에러가 발생
// 이럴 떄 타입 확정하기를 사용함
...
// 단점: 위 예제에서 string을 넘겨도 타입 에러가 발생하지 않기떄문에 as 문법을 쓰면 TS 쓰는 이유가 없어짐
myFunc3('a123')
// 즉, 잘 안씀. 남이 짠 코드를 수정할 떄.. 왜 타입 에러 나는지 모르겠을떄.. 비상용으로만 사용
