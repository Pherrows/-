...
// typed이 길어지거나 타입을 재사용하고 싶다면 변수처럼 만들어 사용가능
...
let ad: TypeAB = { x:1 , y: '2' as never}; // never 타입은 절대 발생하지 않는 값을 의미
...
type TypeC = { color?: string , size: number , readonly position: number[] };
...
type TypeD = { name: string , phone: number , email: string};
const myInfo: TypeD = {
  name: 'zzz',
  phone: 4654646,
  email: 'zzdsafas'
...
type TypeE = { a: boolean};

type TypeF = TypeD & TypeE;

let typeEF: TypeF = { name: 'lee', a: false , phone: 123, email: 'aaaaa' };
...
function i(params: "가위" | '바위' | '보'): ('가위' | '바위' | '보')[] {
  return ["바위", "가위", "보"]
}
