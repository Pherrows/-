...
// 1. boolean
// true 또는 false 값을 지정하는 타입
let active: boolean = true;
// 이 변수에는 bollean 타입만 들어올 수 있음
...
// 이번수에는 객체만 들어올 수 있는데 name은 string, age는 number 타입만 가능

let person3: {name: string, age: number, job?: string} = { name: "Lee", age:32 };
//  ? 사용시 job 속성은 옵셔널
...
let vest: {title: string , name: string} = {title:'춤', name: 'ofonof'};
...
//  열거형 내부의 값은 0부터 시작
// 블로그 게시물의 상태를 표현할 떄 2가 아닌 active라고 하는 것이 가독성이 좋음

// 다음과 같이 시작값을 지정하여 열거형의 숫자르 원하는 값부터 할당할 수도 있음
enum Week {
  MON = 1,
  TUE,
  WED,
...
// 자바스크립트에서 기존 코드들을 활용하면서 부분적으로 타입스크립트를 적용할 떄 사용
...
// 이 함수는 객체를 받아서 데이터베이스에 저장하지만 아무것도 반환하지 않기 떄문에 리턴 타입을 void로 지정
...
// => null 및 undefined 값만 할당할 수 있기 떄문
// 주로 유니언 자료형을 만들떄 사용(예: string |(또는) undefined)
...
let unionValue3: { name: string | number} = {name: 'lee'};
