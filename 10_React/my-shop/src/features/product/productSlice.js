...
export const getMoreProductsAsync = createAsyncThunk(
  `product/getMoreProductsAsync`,// 첫번쨰 인자값: action type(개발자 임의로 작성)
  async () => {  // 두번쨰 인자값: action이 발생했을 떄 실행할 비동기 작업(api 요청 같은)
    const result = await getProducts(); // 비동기 함수 실행 시 `pending` 상태
    return result; // 값을 반환하면 'fulfilled' 상태로 바뀌고 action.payload 에담겨 리듀서 함수로 전달됨
  } 
);
...
const productSlice = createSlice({
  ...
  // (참고)
  // reducers로 동기 작업을 할 떄는 액션 생성 함수를 자동으로 만들어주는 반면
  // extraReducers로 비동기 작업을 할 떄는 액션 생성 함수를 자동으로 만들지 못함
  extraReducers: (builder) => {
    builder
    .addCase(getMoreProductsAsync.pending, (state) => { // pending 상태일떄 동작할 리듀서 함수
      state.status = 'loading';
    })
    .addCase(getMoreProductsAsync.fulfilled, (state, action) => { // fulfilled 상태일떄 동작할 리듀서 함수
      state.status = 'idle'; // complete, success 등
      state.productList.push(...action.payload)
    })
    .addCase(getMoreProductsAsync.rejected, (state) => { // rejected 상태일떄 동작할 리듀서 함수
      state.status = 'fail';
    })
