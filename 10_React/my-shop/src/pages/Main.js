...
function Main(props) {
  ...
  const status = useSelector(selectStatus) // api 요청 상태 (로딩상태)
  ...
  const handelGetMoreProducts = async () => {
    const result = await getProducts();
    if (!result) return; // 결과 값이 없으면 함수 종료
    dispatch(getMoreProducts(result));
  };

  const handelGetMoreProductsAsync = () => {
    ...
    <>
      ...
      <section>
        ...
        <Button variant='secondary' className='mb-4'
          onClick={() => {
            axios.get('https://my-json-server.typicode.com/Pherrows/db-shop/products')
            .then((response) => {
        ...
        <Button variant='secondary' className='mb-4' onClick={handelGetMoreProducts}>
          더보기
        </Button>
        {/* thunk를 이용한 비동기 작업 처리하기 */}
        <Button variant='secondary' className='mb-4' onClick={handelGetMoreProductsAsync}>
          더보기 {status}
        </Button>
...
// 사용 예
// https://my-json-server.typicode.com/Pherrows/db-shop
