...
function FriendStatusEx(props) {
  ...
  useEffect(() => {
    ...
    // 이 함수는 effect 이후에 어떻게 정리 (clean-up)할것인지에 대한 코드가 들어감
    // 이 예제에서는 구독을 해지하는 정리 작업을 함
    // 다른 예로는 타이머를 시작했다면 타이머를 멈추는 정리 작업, 이벤트 리스너를 등록했다면 등록한 리스너를 제거해주는 정리작업
    // componeemtWillUnmpunt 와 같은 방식으로 동작(컴포넌트가 언마운트 될떄 호출함)
    return () => {
      // 언마운트 될떄
      // 구독을 해지하는 API를 호출
