...
function NameForm(props) {
  ...
  const handleSubmit = (e) => {
    alert(`입력한 이름: ` + value)
    e.preventDefault(); // 해당 event 의 기본 동작을 막음
    // 여기서 submit 이벤트의 기본 동작은 새로 고침
  }

  const hamdleChange = (e) => {
    // e.target.value : 이벤트가 발생한 타겟 <input>에 입력된값 = value 속성의 값
    // setValue(e.target.value);
    // 입력값이 state를 통해 관리되는 이런 방식을 제어 컴포넌트라고 부름

    // 만약 ㅏ용자가 입력한 모드 알파뱃을 대문자로 변경시켜서 넣으려면..?
  ...
  <>
    <form onSubmit={handleSubmit}>
      <label>
        이름:
      <input type="text" value={value} onChange={e => hamdleChange(e)}/>
      {/* <input type="text"  onChange={e => hamdleChange(e)}/> */}
