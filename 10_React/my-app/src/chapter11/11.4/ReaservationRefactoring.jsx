...
// 개선한 코드
function ReaservationRefactoring(props) {
  // 만약 여러 개의 state가 서로 관련이 있는 데이터라면 객체 형태로 묶어서 관리가능
  // input에 name 속성을 설정하고 이벤트가 발생했을 떄 이 값을 참조하여 객체 접근
  ...
  const handleInputChange = (e) => {
    ...
    // (inputs가 가르키는 객체의 내부 데이터만 바뀐것이지 참조값은 변하지 않았기 떄문)
    // inputs[name] = inputValue;
    // console.log(inputs);
    // setInputs(inputs); // set함수를 써도 재렌더링을 발생 안함
    // (리액트가 객체가 바뀐것을 감지 못함)

    // 그 대신 새로운 객체를 만들어서 새로운 객체에 변화를 주고 이를 상태로 사용해야함
    // 이러한 작업을 '불변성을 지킨다' 라고부름
    ...
    // 결론 React에서 객체를 업데이트 할 떄에는 기존 객체를 직접 수정하면 안되고
    // 새로운 객체(기존 객체의 복사본)를 만들어서 그객체에 변화를 주고 마지막으로 set함수에 넣어줘야함
...
export default ReaservationRefactoring;