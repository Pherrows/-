...
function App() {
  ...
  // 로컬 스토리지에 저장(주의: DB가 아님, DB 처럼 쓰면 안됨!!)
  // 추가, 수정, 삭제 각 함수에 넣어도 되지만, useEffect()를 활용하면 한번에 처리가능
  useEffect(() => {
  ...
  const handleRemove = useCallback((id) => {
    ...
    // 방법2
    // 불변성을 지키면서 배열의 요소를 제거해야할 떄 filter() 활용
  ...
  // target.checked = !target.checked;
    ...
    // 방법2
    // 불변성을 유지하면서 배열의 특정 요소를 업데이트 해야할 떄 map() 활용
